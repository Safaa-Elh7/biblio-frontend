...
<div class="sidebar w-24 bg-[#7c2d2d] fixed h-full flex flex-col items-center py-6">
    ...
    <div class="sidebar-icon text-white text-2xl mb-6"><a href="{{ route('client.home') }}"><i class="fas fa-home"></i></a></div>
    <div class="sidebar-icon text-white text-2xl mb-6 relative">
        ...
        @if(session()->has('cart') && count(session('cart')) > 0)
            <span class="absolute -top-1 -right-1 bg-[#e6c998] text-[#7c2d2d] rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold">
                {{ count(session('cart')) }}
            </span>
        @endif
    </div>
...
<script>
    // Intercepter les soumissions de formulaire pour ajouter des animations
    document.addEventListener('DOMContentLoaded', function() {
        // Emprunter
        ...
        if (empruntForm) {
            empruntForm.addEventListener('submit', function(e) {
                ...
                if (button) {
                    button.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Ajout...';
